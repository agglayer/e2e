...
setup_file() {
    ...
    contract_bytecode=$(cat $PROJECT_ROOT/tests/ethereum-hardforks/fusaka/contracts/P256Harness.json | jq -r .bytecode.object)
    if [ -z "$contract_bytecode" ]; then
        echo "❌ Failed to read bytecode from $PROJECT_ROOT/tests/ethereum-hardforks/fusaka/contracts/P256Harness.json" >&3
        exit 1
    fi
