...
setup() {
    ...
    l1_rpc_url=${L1_RPC_URL:-"http://$(kurtosis port print "$enclave" el-1-geth-lighthouse rpc)"}
    ...
    l2_rpc_url=${L2_RPC_URL:-"$(kurtosis port print "$enclave" op-el-1-op-geth-op-node-001 rpc)"}
    l2_bridge_addr=${L2_BRIDGE_ADDR:-"0x2E0309f8dDD1EeC27D41e623d73b25cfC4Bb4803"}

    bridge_service_url=${BRIDGE_SERVICE_URL:-"$(kurtosis port print "$enclave" zkevm-bridge-service-001 rpc)"}
    ...
    agglayer_rpc_url=${AGGLAYER_RPC_URL:-"$(kurtosis port print "$enclave" agglayer aglr-readrpc)"}
