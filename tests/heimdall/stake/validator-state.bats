...
#
# RUN: bats tests/heimdall/stake/validator-state.bats
...
setup_file() {
    ...
    if [[ -z "${probe}" || ! "${probe}" =~ ^[0-9]+$ || "${probe}" -eq 0 ]]; then
        echo "WARNING: Heimdall stake/validators endpoint not reachable or returned no validators at ${L2_CL_API_URL} — all stake tests will be skipped." >&3
        echo "1" > "${BATS_FILE_TMPDIR}/heimdall_stake_unavailable"
    else
        echo "Heimdall stake/validators reachable; active validators=${probe}" >&3
...
# Tries /stake/validators first, then the gRPC-gateway /v1beta1/ prefix.
...
@test "heimdall stake: validator set count is non-zero and consistent with CometBFT" {
    ...
    heimdall_count=$(printf '%s' "${vals_json}" \
        | jq '[.[] | select(.jailed != true and (.power // .voting_power // 0 | tonumber) > 0)] | length' \
        2>/dev/null || true)
