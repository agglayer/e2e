...
@test "Process bridge scenarios" {
  while read -r scenario; do
    ...
    case "$testToken" in
      ...
      "WETH") testCommand="$testCommand --token-address $pp2_weth_address" ;;
    ...
    run bash -c "$testCommand"
    [[ "$status" -eq 0 ]] || echo "Failed command: $testCommand"
...
@test "Claim everything on all networks" {
  for rpc_url in $l1_rpc_url $l2_rpc_url; do
    private_key_for_tx=$([[ "$rpc_url" = "$l1_rpc_url" ]] && echo "$l1_private_key" || echo "$l2_private_key")
    bridge_addr_for_tx=$([[ "$rpc_url" = "$l1_rpc_url" ]] && echo "$l1_bridge_addr" || echo "$l2_bridge_addr")
    run polycli ulxly claim-everything \
      --bridge-address "$bridge_addr_for_tx" \
      --bridge-service-map "$l2_network_id=$bridge_service_url" \
      --destination-address "$l1_eth_address" \
      --private-key "$private_key_for_tx" \
      --rpc-url "$rpc_url" \
      --bridge-limit 1000 --bridge-offset 0 \
      --claim-tx-manager-address "$claimtxmanager_addr" \
      --claim-wait-duration "$claim_wait_duration"
    [[ "$status" -eq 0 ]] || echo "Failed claim-everything on $rpc_url"
  done
}