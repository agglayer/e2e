...
# =============================================================================
# Bridge Tests Helper - Auto-Discovery Network Registry
# =============================================================================
#
# This script uses auto-discovery to find networks from environment variables.
# No hardcoded network mappings needed - just add env vars!
# 
# USAGE EXAMPLES:
# 
# 1. Networks are auto-discovered from environment variables:
#    - BALI_NETWORK_68_RPC_URL -> discovers network ID 68 for bali environment
#    - CARDONA_NETWORK_52_RPC_URL -> discovers network ID 52 for cardona
#    - SPEC_NETWORK_1_RPC_URL -> discovers network ID 1 for spec
#    - SEPOLIA_RPC_URL -> discovers network ID 0 (L1)
#
# 2. To add a new network, just add env vars to .env file:
#    BALI_NETWORK_99_RPC_URL="https://..."
#    BALI_NETWORK_99_BRIDGE_ADDR="0x..."
#    BALI_NETWORK_99_PRIVATE_KEY="0x..."
#    BALI_NETWORK_99_BRIDGE_SERVICE_URL="https://..."
#    
#    That's it! The network is automatically discovered and available.
#
# 3. Debug mode to see discovered networks:
#    export DEBUG_NETWORK_REGISTRY=true
#
# See NETWORK_MIGRATION.md for detailed migration guide.
#
# =============================================================================
...
# Source the network registry (auto-discovery)
# shellcheck disable=SC1091
...
deploy_buggy_erc20() {
    ...
    deterministic_deployer_code=$(cast code --rpc-url "$rpc_url" "$deterministic_deployer_addr")

    if [[ $deterministic_deployer_code == "0x" ]]; then
        _log_file_descriptor "3" "Deploying missing proxy contract..."
    ...
    test_erc20_buggy_addr=$(cast create2 --salt $salt --init-code $erc20_buggy_bytecode"$constructor_args" | awk '{ print $1 }')
...
_setup_ephemeral_accounts_in_bulk() {
    ...
    # Determine ETH funding amount based on network_id
    local eth_amount
    if [[ "$network_id" == "0" ]]; then
        eth_amount="100000000000000000"  # 0.1 ETH for network 0
    else
        eth_amount="1000000000000000"   # 0.001 ETH for other networks
    fi
    ...
    if [[ "$custom_gas_token_addr" != "0x0000000000000000000000000000000000000000" ]]; then
        ...
        # Fund ephemeral accounts with ETH. The seed gets parsed to seed_index_YYYYMMDD (e.g., "ephemeral_test_0_20241010") which is identical to the seed being used in the bridge-tests-suite.
        local eth_fund_output
        if ! eth_fund_output=$(polycli fund --rpc-url "$target_rpc_url" --number "$total_scenarios" --private-key "$target_private_key" --file /tmp/wallets-funded.json --seed "ephemeral_test" --eth-amount "$eth_amount" 2>&1); then
