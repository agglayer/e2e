...
@test "L1 → PP3 (native/WETH) → PP1" {
    ...
    process_bridge_claim "$l1_rpc_network_id" "$bridge_tx_hash_pp3" "$rollup_3_network_id" "$l2_bridge_addr" "$aggkit_bridge_3_url" "$aggkit_bridge_3_url" "$l2_rpc_url_3"
    ...
    process_bridge_claim "$rollup_3_network_id" "$bridge_tx_hash" "$rollup_1_network_id" "$l2_bridge_addr" "$aggkit_bridge_3_url" "$aggkit_bridge_1_url" "$l2_rpc_url_1"
...
@test "L1 → PP1 (custom gas token) → PP2" {
    ...
    local initial_receiver_balance_pp1=$(cast balance "$receiver" --rpc-url "$l2_rpc_url_1")
    ...
    local final_receiver_balance_pp1=$(cast balance "$receiver" --rpc-url "$l2_rpc_url_1")
    ...
    run verify_balance "$l2_rpc_url_1" "$native_token_addr" "$destination_addr" "$initial_receiver_balance_pp1" "$amount"
    assert_success
    ...
    run verify_balance "$l2_rpc_url_2" "$pp2_wrapped_token_addr" "$destination_addr" 0 "$amount"
    assert_success
}
...
@test "L1 → PP1 (native) → PP3" {
    ...
    process_bridge_claim "$l1_rpc_network_id" "$bridge_tx_hash_pp1" "$rollup_1_network_id" "$l1_bridge_addr" "$aggkit_bridge_1_url" "$aggkit_bridge_1_url" "$l2_rpc_url_1"
    echo "==== 💰 Verifying balance on PP1" >&3
    run verify_balance "$l2_rpc_url_1" "$weth_token_rollup_1" "$destination_addr" "$weth_token_init_sender_balance_pp1" "$amount"
    assert_success

    # Set receiver address and query for its initial native token addr balance on the PP1
    local initial_receiver_balance_pp3=$(cast balance "$receiver" --rpc-url "$l2_rpc_url_3")
    ...
    process_bridge_claim "$rollup_1_network_id" "$bridge_tx_hash" "$rollup_3_network_id" "$l2_bridge_addr" "$aggkit_bridge_1_url" "$aggkit_bridge_3_url" "$l2_rpc_url_3"

    echo "==== 💰 Verifying balance on PP3" >&3
    run verify_balance "$l2_rpc_url_3" "$native_token_addr" "$destination_addr" "$initial_receiver_balance_pp3" "$amount"
    assert_success
}

@test "L1 → PP1 (custom gas token) → PP3 -> PP2" {
    # Set receiver1 address and query for its initial native token addr balance on the PP1
    local initial_receiver1_balance_pp1=$(cast balance "$receiver1_addr" --rpc-url "$l2_rpc_url_1")
    ...
    local final_receiver1_balance_pp1=$(cast balance "$receiver1_addr" --rpc-url "$l2_rpc_url_1")
    echo "Final receiver1 ($receiver1_addr) balance of gas token addr on PP1 $final_receiver1_balance_pp1" >&3

    echo "==== 💰 Verifying balance on PP1" >&3
    run verify_balance "$l2_rpc_url_1" "$native_token_addr" "$destination_addr" "$initial_receiver1_balance_pp1" "$amount"
    assert_success

    # Set receiver1 address and query for its initial native token addr balance on the PP2
    local initial_receiver1_balance_pp2=$(cast balance "$receiver1_addr" --rpc-url "$l2_rpc_url_2")
    ...
    echo "==== 💰 Verifying balance on PP2" >&3
    run verify_balance "$l2_rpc_url_2" "$native_token_addr" "$destination_addr" "$initial_receiver1_balance_pp2" "$amount"
    assert_success
    ...
    echo "==== 💰 Verifying balance on PP3" >&3
    run verify_balance "$l2_rpc_url_3" "$pp3_wrapped_token_addr" "$destination_addr" 0 "$amount"
    assert_success
    ...
    amount="0.01ether"
    meta_bytes="0x"
    run bridge_asset "$pp3_wrapped_token_addr" "$l2_rpc_url_3" "$l2_bridge_addr"
    assert_success
    local bridge_tx_hash=$output
