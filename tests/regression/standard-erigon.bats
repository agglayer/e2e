...
@test "send malformed PUSH opcode" {
    ...
    local addr=$(cast wallet address --private-key $private_key)
    local nonce=$(cast nonce --rpc-url $l2_rpc_url $addr)
    local test_address=$(cast compute-address --rpc-url $l2_rpc_url --nonce $nonce $addr | sed 's/.*: //')
...
@test "send executable PUSH operand" {
    # this is a manually constructed contract
    ...
    # 0000000b: JUMP
    # 0000000c: INVALID
    # In most EVMs, the JUMP should fail because offset 1 is inside of a push. It's not typically a valid jumpdest. But in this case in zkevm it should work, and we sure return properly and never hit the INVALID
    ...
    if [[ $(jq -r '.logs | length' $out_file) -ne 1 ]]; then
        echo "expected to have one log in executable push operand"
        exit 1
    fi
...
# cast rpc --rpc-url http://127.0.0.1:33194 debug_traceTransaction 0x17228bea2922d97a3ace05f305f07693a3dc85f894c1fe9d9688f298896f5441 | jq -c '.structLogs[]'
...
@test "counter overflowing transactions do not create new batches" {
    ...
    local eth_address=$(cast wallet address --private-key "$private_key")
    local nonce=$(cast nonce --rpc-url "$l2_rpc_url" "$eth_address")

    local wallets=$(mktemp)
    local nonce_file=$(mktemp)

    cast wallet new --number 15 --json > $wallets

    jq -r '.[].address' $wallets | while read tmp_address; do
    ...
    jq -r '.[].private_key' $wallets | while read tmp_private_key; do
        cast send --async \
             --gas-limit 29999999 \
             --legacy \
