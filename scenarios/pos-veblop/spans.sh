...
# Track previous span's end block to detect skips
prev_end_block=""
...
for ((span_id=latest_span_id; span_id>=1; span_id--)); do
  ...
  if [[ $? -eq 0 && "$span_data" != *"error"* ]]; then
    ...
    # Determine status - check if this span was skipped
    status="Normal"
    if [[ -n "$prev_end_block" ]]; then
    ...
    # Update previous end block for next iteration
