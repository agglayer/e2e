...
# urls etc
pos_rpc_url=$(kurtosis port print $kurtosis_enclave_name l2-el-1-bor-heimdall-v2-validator rpc)
pos_rpc_url_kurtosis="http://l2-el-1-bor-heimdall-v2-validator:8545"
...
hacker_pkey=$(echo "$hacker_wallet_json" | jq -r '.[0].private_key')

# fund hacker on pos with 1 eth
cast send  --legacy --rpc-url $pos_rpc_url --value 1ether --private-key $l1_preallocated_private_key $hacker_addr

l2_rpc_url=$(kurtosis port print $kurtosis_enclave_name op-el-1-op-geth-op-node-001 rpc)

hacker_balance_r0=$(cast balance --rpc-url $l1_rpc_url $hacker_addr)
hacker_balance_r1=$(cast balance --rpc-url $l2_rpc_url $hacker_addr)
hacker_balance_r2=$(cast balance --rpc-url $pos_rpc_url $hacker_addr)

echo "Hacker address=$hacker_addr, balance on L1=$hacker_balance_r0, on OP=$hacker_balance_r1, on PoS=$hacker_balance_r2"
...
cast send \
    --legacy \
    --rpc-url $pos_rpc_url \
    --private-key $hacker_pkey \
    $fake_bridge_addr \
    'maliciousBridgeAsset(uint32,address,uint256,address,bool)' \
    1 \
    $hacker_addr \
    $(echo 10 | cast to-wei) \
    $(cast address-zero) \
