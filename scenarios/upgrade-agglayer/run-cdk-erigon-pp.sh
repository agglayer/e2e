...
kurtosis run \
         --enclave "$kurtosis_enclave_name" \
         --args-file ./cdk-erigon-pp-single-network.yml \
         "github.com/0xPolygon/kurtosis-cdk@$kurtosis_hash"
...
run_lxly_bridging "CDK-Erigon PP Bridging" "cdk-erigon-rpc-001" "zkevm-bridge-service-001" "0x1359D1eAf25aADaA04304Ee7EFC5b94C43e0e1D5" "$l2_bridge_address" "12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625" > ./scenarios/upgrade-agglayer/cdk-erigon-pp-bridging.log 2>&1 & pids+=($!)
...
# Update rolluptype
cast send "$rollup_manager_address" "updateRollupByRollupAdmin(address,uint32)" "0x1Fe038B54aeBf558638CA51C91bC8cCa06609e91" "$rollup_type_id" --rpc-url http://$(kurtosis port print $kurtosis_enclave_name el-1-geth-lighthouse rpc) --private-key "0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625"

# Save Rollup Information to a file.
cast call --json --rpc-url http://$(kurtosis port print $kurtosis_enclave_name el-1-geth-lighthouse rpc) "$rollup_manager_address" 'rollupIDToRollupData(uint32)(address,uint64,address,uint64,bytes32,uint64,uint64,uint64,uint64,uint64,uint64,uint8)' "1" | jq '{"sovereignRollupContract": .[0], "rollupChainID": .[1], "verifier": .[2], "forkID": .[3], "lastLocalExitRoot": .[4], "lastBatchSequenced": .[5], "lastVerifiedBatch": .[6], "_legacyLastPendingState": .[7], "_legacyLastPendingStateConsolidated": .[8], "lastVerifiedBatchBeforeUpgrade": .[9], "rollupTypeID": .[10], "rollupVerifierType": .[11]}' > ./rollup-out.json
...
run_lxly_bridging "CDK-Erigon PP Bridging" "cdk-erigon-rpc-001" "zkevm-bridge-service-001" "0x1359D1eAf25aADaA04304Ee7EFC5b94C43e0e1D5" "$l2_bridge_address" > ./scenarios/upgrade-agglayer/cdk-erigon-pp-bridging.log 2>&1 & pids+=($!)
