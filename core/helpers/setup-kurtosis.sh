...
if [[ "${PACKAGE}" == "kurtosis-cdk" ]]; then
    ...
    if [[ "${ARGS_FILE}" == "*heimdall-v2*" ]]; then
        ...
        export_env_var "L2_RPC_URL" "$(kurtosis port print "${ENCLAVE}" "l2-el-1-bor-modified-for-heimdall-v2-heimdall-v2-validator" rpc)"
        export_env_var "L2_CL_API_URL" "$(kurtosis port print "${ENCLAVE}" "l2-cl-1-heimdall-v2-bor-modified-for-heimdall-v2-validator" http)"
    else
